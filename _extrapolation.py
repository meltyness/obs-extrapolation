...
peeky.proportional_control = 0.08
peeky.push_scale = -2


# cv2.namedWindow("Debug Output")
# cv2.waitKey(1)
print("loaded ffi script i guess")
...
def manage_imgproc(data, w, h):
    ...
    # Display the broken fucking debugger piece of shit
    ...
    if (not peeky.kp_last is None) and (not peeky.des is None) and (not peeky.des_last is None):
        ...
        if peeky.jump_limiter > 0:
            peeky.disturbance.x += min(peeky.jump_limiter, peeky.jump_max)
            peeky.jump_limiter -= min(peeky.jump_limiter, peeky.jump_max)
        else:
            peeky.disturbance.x -= min(peeky.jump_limiter, peeky.jump_max)
            peeky.jump_limiter += min(abs(peeky.jump_limiter), peeky.jump_max)
        ...
        if abs(peeky.disturbance.x - peeky.init_vec.x) > 5:
            peeky.disturbance.x -= (peeky.disturbance.x - peeky.init_vec.x) * peeky.proportional_control
...
def event_loop():
    ...
    if peeky.duration > peeky.start_delay:
        ...
        if peeky.tick_acc > peeky.interval_sec:
            # print("Event Looped into Callback")
            peeky.callback()
            peeky.tick_acc = 0
            print("leavin the clab")
            return
    else:
